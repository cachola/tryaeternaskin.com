...
class Http
{
    ...
    {
        TmpLogger::logdev('codebase', 'POST:' . $url . PHP_EOL . print_r($params, true) . PHP_EOL);
 
        // if (
        //     strpos($url, '201clicks.com') !== false &&
        //     is_array($params) && !empty($_SERVER)
    ...
    private static function customRequest($url, $params, $headers = array(), $method, $curlOptions = array())
    {
        TmpLogger::logdev('codebase','CUSTOM:' . $url . PHP_EOL . print_r($params, true) . PHP_EOL);
        $headerStrings = self::makeHeaders($headers);
    ...
    private static function makeRequest()
    {
        ...
        if ( env('DEV_CURL_PROXY',false)) {
            curl_setopt($ch, CURLOPT_PROXY, '127.0.0.1');
            curl_setopt($ch, CURLOPT_PROXYPORT, '8900');
        ...
        TmpLogger::logdev('codebase','RESP:' . self::$response . PHP_EOL);
