...
class Responsecrm extends BaseCrm
{
    ...
    protected function newOrder()
    {
        ...
        if ($this->params['meta.isDownsellStep'] || $this->params['meta.isPrepaidFlow'])
        {
            $this->prospect();
    ...
    private function makeHttpRequest()
    {
        ...
        $headers = array('Content-Type' => 'application/json', 'Authorization' => 'ApiGuid ' . $this->username);
    ...
    private function prepareResponse($response)
    {
        ...
        if (empty($this->response) || $this->response['Status'] != 0)
        {
            CrmResponse::replace(array(
                'success' => false,
                'errors' => array(
                    'crmError' => (!empty($this->response['ErrorMessage'][1]) ? $this->response['ErrorMessage'][1] : !empty($this->response['ErrorMessage'][0]) ? $this->response['ErrorMessage'][0] : ''),
                ),
            ));
            return false;
        }
    ...
    {
        unset($this->params['prospectId'], $this->params['firstName'], $this->params['lastName'], $this->params['userIsAt'], $this->params['billingSameAsShipping'], $this->params['billingFirstName'], $this->params['billingLastName'], $this->params['billingAddress1'], $this->params['billingZip'], $this->params['billingCity'], $this->params['billingState'], $this->params['billingCountry'], $this->params['cardType'], $this->params['cardNumber'], $this->params['SiteID'], $this->params['FirstName'], $this->params['LastName'], $this->params['Email'], $this->params['Phone'], $this->params['Address1'], $this->params['Address2'], $this->params['ZipCode'], $this->params['City'], $this->params['State'], $this->params['CountryISO'], $this->params['campaignId'], $this->params['ipAddress']);
    }
