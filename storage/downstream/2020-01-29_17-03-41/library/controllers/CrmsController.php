...
class CrmsController
{
    ...
    private function preparePayload($isSplitOrder = false)
    {
        ...
        if (!empty($payload['multiple_split']))
        {
            CrmPayload::update(array('multiple_split' => $payload['multiple_split']));
        }
    ...
    private function multiSplit()
    {

        $multiSplitConfig = Config::configurations(sprintf(
                                '%s', $this->currentConfigId
        ));
        if (!$multiSplitConfig['multi_split'] ||
                empty(json_decode($multiSplitConfig['multi_split_campaign_ids'], true)))
        {
            return;
        }
        $multiSplitCampaigns = json_decode($multiSplitConfig['multi_split_campaign_ids'], true);
        $products = array();
        foreach ($multiSplitCampaigns as $campaignId)
        {
            $product = Campaign::find($campaignId);
            array_push($products, $product);
        }
        CrmPayload::update(
                array('multiple_split' => array(
                        'multi_split' => true,
                        'multi_split_campaign_ids' => json_decode($multiSplitConfig['multi_split_campaign_ids'], true),
                        'enable_related_delay' => $multiSplitConfig['enable_related_delay'],
                        'related_delay_input' => $multiSplitConfig['related_delay_input'],
                        'products' => $products
                    )
                )
        );
