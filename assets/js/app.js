...
(function ($) {
    $(function () {
        ...
        $('form[name=fullprospect_form]').formHandler({
            type: 'fullprospect',
            errorModal: true,
        ...
        $('form[name=shipping_form]').formHandler({
            type: 'shipping',
            errorModal: true,
            autoFillFormElement: 'shipping_form1', // form name only
            countryDropdown: 'Select Country',
            ajaxLoader: {
                div: '#loading-indicator',
                timeInOut: 0
...
window.addEventListener('popstate', function (event) {
    if(app_config.enable_browser_back_button && 
        app_config.pageType == 'checkoutPage'
    ){
