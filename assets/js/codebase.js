var cb = (function($) {
    ...
    var shippingFormFields = {

                // moved fields from prospect
        shippingAddress1: {},
        shippingAddress2: {},
        shippingZip: {
            validators: [
    ...
    var fullProspectForm = {
        data: {},
        fields: fullProspectFormFields,
        errors: {}
    };
    var shippingForm = {
        data: {},
    ...
    function valdateCardNumber(fieldName, inputElement, form, validationMeta) {
        ...
        if (typeof app_config.allowed_tc !== 'undefined' && app_config.allowed_tc.length) {
            $(app_config.allowed_tc).each(function(key, value) {
    ...
    function loadForm(formElement, formType) {
        ...
        switch (formType) {
            case 'prospect':
                ...
                break;
            case 'fullprospect':
                form = $.extend(true, {}, fullProspectForm);
                break;
            case 'checkout':
    ...
    function getUI(errors) {
        ...
        if(window.paypalClick){
           html += '<p style="background: bisque;padding: 10px;border: black 1px solid;">In order to proceed with PayPal you will first need to complete the customer information fields on this page. Please fill out that information before clicking the PayPal button.</p><br><br>';
        }    
        html += '<ul>' + li + '</ul></div>';
        html += '</div>';
        return html;
    ...
    function submitForm(formElement, formOptions) {
        ...
        $.ajax({
            ...
            method: 'post',
            data: formElement.serialize() + testFlag,
            beforeSend: function() {
